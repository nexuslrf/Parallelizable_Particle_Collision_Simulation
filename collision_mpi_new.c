...
void gen_colli_type(MPI_Datatype *Type_ptr)
{
    ...
    disp[2] = offsetof(Collision, id);
    disp[3] = offsetof(Collision, time);
    MPI_Type_create_struct(4, blocklen, disp, type, Type_ptr);
    MPI_Type_commit(Type_ptr);
}
...
void master()
{
    ...
    */
    for(step = 0; step < s; step++)
    {
        ...
        for(k=0;k<count;k++)
        {
            ...
            { //wall collision
                if(!colli_mat[colli->pb])
                {
                    ...
                    colli_chunk_queue[i][countbuf[i + 1]++]=colli->id;
                }
            }
            else if(!colli_mat[colli->pa]) // p-p collision
            {
                if(!colli_mat[colli->pb])
                {
                    ...
                    m = job_mat[i][j];
                    colli_chunk_queue[m][countbuf[m + 1]++]=colli->id;
                    // if (i!=j)
        ...
        {
            MPI_Send(colli_chunk_queue[i], countbuf[i+1], MPI_INT, i+1, i+1, parti_comm);
        }
...
void slave()
{
    ...
    int send_mat[num_slave][nprocs], job_mat[num_slave][num_slave];
    int countbuf[nprocs], colli_idx_buf[n];
    memset(send_mat[0], 0, num_slave*nprocs*sizeof(int));
    memset(job_mat[0], 0, num_slave*num_slave*sizeof(int));
    ...
    */
    for(step = 0; step < s; step++)
    {
        ...
        MPI_Scatter(countbuf, 1, MPI_INT, &count, 1, MPI_INT, MASTER_ID, parti_comm);
        MPI_Recv(colli_idx_buf, count, MPI_INT, MASTER_ID, myid, parti_comm, &Stat);
        for(k=0; k<count; k++)
        {
            colli = colli_time + colli_idx_buf[k];
            if(colli->pa<0) // wall
            {

...
__managed__ int *colli_mat, *pa_idx, *pb_idx, *colli_queue;
...
__global__ void check_colli(int offset)
{
    // get kernel idx
...
int main(int argc, char** argv)
{
    ...
    host_num_cmp = host_n * (host_n-1) / 2 + host_n;
    ...
    for (step = 0; step < host_s; step++) {
        ...
        real_colli=0;
        /* Call the kernel */
        check_colli<<<num_blocks, num_threads>>>(total_threads);
        /* Barrier */
        ...
        proc_collision<<<num_blocks, num_threads>>>(total_threads);
        /* Barrier */
        cudaDeviceSynchronize();
        for(i=0;i<host_n;i++)
        {

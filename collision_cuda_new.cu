...
int main(int argc, char** argv)
{
    ...
    for (step = 0; step < host_s; step++) {
        ...
        real_colli=0;
        /* Call the kernel */
        check_wall_colli<<<num_blocks, num_threads, 0, stream1>>>(chunk_p); 
        check_pp_colli<<<num_blocks, num_threads, 0, stream2>>>(chunk_c);
        /* Barrier */
        ...
        /* Call the kernel */
        update_particle<<<num_blocks, num_threads, 0, stream1>>>(chunk_p);
        proc_collision<<<num_blocks, num_threads, 0, stream2>>>((real_colli-1)/total_threads + 1);
        /* Barrier */
        cudaDeviceSynchronize();

...
__global__ void check_wall_colli(int offset)
{
...
__global__ void check_pp_colli(int offset)
{
    // get kernel idx
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    int cnt;
...
__global__ void proc_collision(int offset)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    particle_t *P_a, *P_b;
    Collision *Colli;
...
int main(int argc, char** argv)
{
    ...
    size_t colli_size, queue_size, p_size, cmp_size;
    p_size = sizeof(particle_t) * host_n;
    queue_size = sizeof(int) * host_n;
    cmp_size = sizeof(int) * host_num_cmp;
    cudaMallocManaged((void**)&particles, p_size);
    cudaMallocManaged((void**)&colli_mat, queue_size);
    cudaMemset(colli_mat, 0,  queue_size);
    cudaMallocManaged((void**)&colli_queue, queue_size);
    cudaMallocManaged((void**)&colli_time, sizeof(Collision) *(host_num_cmp+host_n));
    cudaMallocManaged((void**)&pa_idx, cmp_size);
    cudaMallocManaged((void**)&pb_idx, cmp_size);
    j = 0;
    ...
    cudaMemPrefetchAsync(particles, p_size, deviceId);
    ...
    cudaMemPrefetchAsync(pa_idx, cmp_size, deviceId);
    cudaMemPrefetchAsync(pb_idx, cmp_size, deviceId);
    ...
    for (step = 0; step < host_s; step++) {
        ...
        real_colli=0;
        /* Call the kernel */
        check_wall_colli<<<num_blocks, num_threads, 0, stream1>>>(total_threads); 
        check_pp_colli<<<num_blocks, num_threads, 0, stream2>>>(total_threads);
        /* Barrier */
        cudaDeviceSynchronize();
        // Prefetch to CPU
        colli_size = count*sizeof(Collision);
        cudaMemPrefetchAsync(colli_time, colli_size, cudaCpuDeviceId);
        // find real collisions
        qsort(colli_time, count, sizeof(Collision), compare);
        find_real_collisions();
        /* Call the kernel */
        cudaMemPrefetchAsync(colli_time, colli_size, deviceId);
        cudaMemPrefetchAsync(colli_queue, queue_size, deviceId);
        cudaMemPrefetchAsync(colli_mat, queue_size, deviceId);
        update_particle<<<num_blocks, num_threads, 0, stream1>>>(total_threads);
        proc_collision<<<num_blocks, num_threads, 0, stream2>>>(total_threads);
        /* Barrier */
        cudaDeviceSynchronize();
        if(mode==MODE_PRINT)
            // cudaMemPrefetchAsync(particles, p_size, cudaCpuDeviceId);
